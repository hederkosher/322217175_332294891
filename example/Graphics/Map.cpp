...
static void PlaceDepots() {
	...
	for (int d = 0; d < 2; d++) {
		...
		do {
			r = rand() % numRooms;
			attempts++;
		} while (attempts < 100 && (r == usedRooms[0] || r == usedRooms[1] || r == usedRooms[2] || r == usedRooms[3]));
...
static void PlaceTroops(NPC** team1, NPC** team2) {
	...
	FindSpawnInRoom(rooms[r1], sx, sy);
	team1[3] = new LogisticNPC(sx, sy, 'P', 1, LOGISTIC_1);
	...
	FindSpawnInRoom(rooms[r2], sx, sy);
	team2[3] = new LogisticNPC(sx, sy, 'P', 2, LOGISTIC_2);
